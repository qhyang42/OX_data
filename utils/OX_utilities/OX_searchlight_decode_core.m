function results = OX_searchlight_decode_core(subjidx, target, varargin)
%OX_SEARCHLIGHT_DECODE_CORE Shared implementation for context/odor searchlights.
%
% This function is intended to be called through OX_searchlight_context or
% OX_searchlight_odor.
%
...
%
% Output NIfTIs include accuracy, accuracy-minus-chance, uncorrected p,
% FDR q, max-stat FWE p, and q<.05 / FWE<.05 masks.
...
fprintf('Trials: %d | Runs: %d | Classes: %d | Chance: %.4f | CV: %s\n', ...
    nTrials, nRuns, nClasses, chance, cv_method);
...
%% Observed decoding
fprintf('Running observed %s %s decoding...\n', cv_method, target);
tic;
if use_parallel
    model_const = parallel.pool.Constant(modelmd);
    model_cleanup = onCleanup(@() delete(model_const)); %#ok<NASGU>
    [voxel_acc, confusion_matrices] = decode_searchlight_map_parallel( ...
        model_const, neighborhoods, y, cv_ids, is_loo, nClasses, ...
        opts.MinVoxels, opts.DemeanPatterns, opts.ScaleVoxels);
else
    [voxel_acc, confusion_matrices] = decode_searchlight_map_serial( ...
        modelmd, neighborhoods, y, cv_ids, is_loo, nClasses, ...
        opts.MinVoxels, opts.DemeanPatterns, opts.ScaleVoxels);
end
fprintf('Observed map complete in %.1f minutes.\n', toc / 60);

valid_centers = isfinite(voxel_acc);
assert(any(valid_centers), 'No searchlights produced a finite accuracy value.');

%% Full-map permutation inference
p_uncorrected = nan(size(voxel_acc));
q_fdr = nan(size(voxel_acc));
p_fwe = nan(size(voxel_acc));
sig_fdr05 = false(size(voxel_acc));
sig_fwe05 = false(size(voxel_acc));
max_null = [];
fwe_accuracy_threshold05 = NaN;
...
if opts.NumPermutations > 0
    ...
    exceed_count = zeros(size(voxel_acc), 'uint32');
    ...
    for perm_idx = 1:opts.NumPermutations
        ...
        if use_parallel
            perm_acc = decode_searchlight_map_parallel( ...
                model_const, neighborhoods, y_perm, cv_ids, is_loo, nClasses, ...
                opts.MinVoxels, opts.DemeanPatterns, opts.ScaleVoxels);
        else
            perm_acc = decode_searchlight_map_serial( ...
                modelmd, neighborhoods, y_perm, cv_ids, is_loo, nClasses, ...
                opts.MinVoxels, opts.DemeanPatterns, opts.ScaleVoxels);
        end

        comparable = valid_centers & isfinite(perm_acc);
        exceed_count(comparable) = exceed_count(comparable) + ...
            uint32(perm_acc(comparable) >= voxel_acc(comparable));
        max_null(perm_idx) = max(perm_acc(comparable) - chance);
    ...
    observed_effect = voxel_acc(valid_centers) - chance;
    max_null_sorted = sort(max_null);
    for vi = find(valid_centers)'
        p_fwe(vi) = (1 + sum(max_null >= (voxel_acc(vi) - chance))) / ...
    ...
    % accuracy metric and chance level. The p_FWE map remains the preferred
    % output for reporting.
    threshold_index = min(opts.NumPermutations, ...
        max(1, ceil(0.95 * (opts.NumPermutations + 1))));
    fwe_accuracy_threshold05 = chance + max_null_sorted(threshold_index);

    fprintf('Permutation inference complete. FDR q<.05: %d centers | max-stat FWE p<.05: %d centers\n', ...
        sum(sig_fdr05), sum(sig_fwe05));
    fprintf('Max-stat FWE .05 accuracy threshold: %.4f\n', fwe_accuracy_threshold05);
end

%% Package results
if strlength(string(opts.OutputDir)) == 0
    output_dir = fullfile(base_outdir, sprintf('searchlight_%s_%s', target, cv_short_name));
else
...
results.accuracy = voxel_acc;
results.confusion_matrices = confusion_matrices;
results.confusion_matrix_dimension_order = {'searchlight_center', 'true_class', 'predicted_class'};
results.accuracy_minus_chance = voxel_acc - chance;
...
results.fwe_accuracy_threshold05 = fwe_accuracy_threshold05;
...
results.classifier = 'nearest training-class template by Pearson correlation';
...
if opts.SaveOutputs
    ...
    prefix = sprintf('%s_subj%d', target, subjidx);

    write_float_map(gmmask, size(gm_mask), center_linear_idx, voxel_acc, ...
        fullfile(output_dir, [prefix '_accuracy.nii']));
    write_float_map(gmmask, size(gm_mask), center_linear_idx, voxel_acc - chance, ...
        fullfile(output_dir, [prefix '_accuracy_minus_chance.nii']));
...
%% ------------------------------------------------------------------------
function [accuracy, confusion_matrices] = decode_searchlight_map_parallel(model_const, neighborhoods, y, cv_ids, is_loo, nClasses, min_voxels, demean_patterns, scale_voxels)
nCenters = numel(neighborhoods);
accuracy = nan(nCenters, 1);
confusion_matrices = [];
if nargout > 1
    confusion_matrices = nan(nCenters, nClasses, nClasses);
    parfor vi = 1:nCenters
        [accuracy(vi), center_confusion] = decode_one_searchlight( ...
            model_const.Value, neighborhoods{vi}, y, cv_ids, is_loo, nClasses, ...
            min_voxels, demean_patterns, scale_voxels);
        confusion_matrices(vi, :, :) = reshape(center_confusion, [1, nClasses, nClasses]);
    end
else
    parfor vi = 1:nCenters
        accuracy(vi) = decode_one_searchlight( ...
            model_const.Value, neighborhoods{vi}, y, cv_ids, is_loo, nClasses, ...
            min_voxels, demean_patterns, scale_voxels);
    end
end
...
%% ------------------------------------------------------------------------
function [accuracy, confusion_matrices] = decode_searchlight_map_serial(modelmd, neighborhoods, y, cv_ids, is_loo, nClasses, min_voxels, demean_patterns, scale_voxels)
nCenters = numel(neighborhoods);
accuracy = nan(nCenters, 1);
confusion_matrices = [];
if nargout > 1
...
%% ------------------------------------------------------------------------
function [accuracy, confusion_matrix] = decode_one_searchlight(modelmd, voxel_inds, y, cv_ids, is_loo, nClasses, min_voxels, demean_patterns, scale_voxels)
if nargout > 1
    confusion_matrix = nan(nClasses, nClasses);
end
...
if is_loo
    predictions = decode_leave_one_out(X, y, nClasses, min_voxels, scale_voxels);
else
    predictions = decode_grouped_folds(X, y, cv_ids, min_voxels, scale_voxels);
end

valid_predictions = isfinite(predictions);
if ~all(valid_predictions)
    accuracy = NaN;
else
    accuracy = mean(predictions == y);
    if nargout > 1
        confusion_matrix = accumarray([y, predictions], 1, [nClasses, nClasses]);
    end
end
end

%% ------------------------------------------------------------------------
function predictions = decode_leave_one_out(X, y, nClasses, min_voxels, scale_voxels)
nTrials = size(X, 1);
...
%% ------------------------------------------------------------------------
function predictions = decode_grouped_folds(X, y, cv_ids, min_voxels, scale_voxels)
...
scores = Xtest_centered * templates';
scores(~isfinite(scores)) = -Inf;

[~, best_idx] = max(scores, [], 2);
predictions(valid_tests) = class_labels(best_idx(valid_tests));
end
