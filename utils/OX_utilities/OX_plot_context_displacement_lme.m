...
colors=lines(3); n_rois=numel(O.roi_names); ncols=min(3,n_rois+1);
for c=O.contexts
    ...
    for predictor=O.predictors
        ...
        t=tiledlayout(f,ceil((n_rois+1)/ncols),ncols,'TileSpacing','compact');
        d=figure('Visible','off','Color','w','Position',[50 50 1150 1500]);
        dt=tiledlayout(d,n_rois,2,'TileSpacing','compact');
        for r=1:n_rois
            ...
            if isempty(A)
                title(ax,O.roi_names(r),'Interpreter','none');text(ax,.5,.5,{'Mixed-model inference','unavailable'},'Units','normalized','HorizontalAlignment','center');axis(ax,'off');
                for tile=2*r-1:2*r, da=nexttile(dt,tile);axis(da,'off');text(da,.1,.5,{'Failed fit/inference','See failure table'},'Units','normalized');end
                continue;
            end
            ...
            title(ax,[label;sprintf('beta = %.3f, p = %.3g, q = %.3g',A.beta_standardized,A.p_value,A.q_value)],'Interpreter','none');
        ...
        ax=nexttile(t,n_rois+1); hold(ax,'on'); h=gobjects(4,1);
