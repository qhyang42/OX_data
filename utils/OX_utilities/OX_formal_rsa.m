...
subjects=2:6; rois=["AON","PirF","PirT","olfAMG","olfOFC"];
...
blank=struct('name',"",'predictors',predictors,'beta',nan(5,5,nP), ...
    'null_beta',nan(5,5,nP,nB),'subject_statistics',table(),'group_statistics',table(), ...
...
permutation_mappings=zeros(80,nB,5,'uint8');
provenance=cell(5,1); observations=table();
for s=1:5
    sid=subjects(s); B=load(fullfile(rdmroot,"subj_"+sid+"_behavioral_RDMs.mat"));
    N=load(fullfile(rdmroot,'neural',"subj_"+sid+"_neural_RDMs.mat"),'results'); N=N.results;
    ...
    provenance{s}=struct('subject_id',sid,'preprocessing',N.preprocessing, ...
        'roi_metadata',N.roi_metadata,'neural_input',fullfile(rdmroot,'neural',"subj_"+sid+"_neural_RDMs.mat"), ...
    ...
    for r=1:5
        ...
        for m=1:numel(models)
            ...
            assert(max(abs(ref-b(2:nP+1)'))<1e-10,'Exploratory beta mismatch.');
            ...
            models(m).fit_statistics=[models(m).fit_statistics; ...
                table(sid,rois(r),numel(y),0,rank(A),r2,adj,max(abs(ref-b(2:nP+1)')), ...
                'VariableNames',{'subject_id','roi','n_valid','n_dropped','design_rank','r2','adjusted_r2','exploratory_max_beta_error'})];
        end
        for first=1:250:nB
    ...
    fprintf('[%s] subject %d: %d permutations across five ROIs complete.\n',analysis,sid,nB);
end
for m=1:numel(models)
    model=models(m); assert(all(isfinite(model.null_beta(:))) && all(isfinite(model.beta(:))));
    group_null=reshape(mean(model.null_beta,1),5,nP,nB);
    group_beta=reshape(mean(model.beta,1),5,nP);
    st=table(); gt=table();
    for s=1:5
        part=table();
        for r=1:5
    ...
    for r=1:5
        for j=1:nP
            ...
            assert(max(abs(nb-reshape(sum(model.null_beta(:,r,j,:),1)/5,[],1)))<1e-12);
            gt=[gt;table(rois(r),predictors(j),obs,OX_rsa_empirical_p(obs,nb),mean(nb),std(nb),5,nB, ...
...
results.inference="Two-sided global neural condition-correspondence null; equal-weight mean of five independent subject null draws. Not a coefficient-specific conditional null or a population random-effects test.";
...
for s=1:5
    ...
    for m=1:numel(models)
        ...
        subject_result.models(m).beta=reshape(models(m).beta(s,:,:),5,nP);
        subject_result.models(m).null_beta=reshape(models(m).null_beta(s,:,:,:),5,nP,nB);
