...
parser=argparse.ArgumentParser();parser.add_argument('--with-controls',action='store_true');args=parser.parse_args()
...
q=np.concatenate([false_discovery_control(p[:6],method='bh'),false_discovery_control(p[6:],method='bh')]) if args.with_controls else false_discovery_control(p,method='bh')
assert len(p)==len(order) and np.isfinite(p).all()
...
fig,ax=plt.subplots(figsize=(14 if args.with_controls else 11,7.3));fig.subplots_adjust(left=.12,right=.97,top=.80,bottom=.27)
box=ax.boxplot(values,positions=np.arange(len(order)),widths=.52,patch_artist=True,showfliers=False,
...
for i,(label,key) in enumerate(order):
 for row in subject_table:
  ...
  ax.scatter(i+offsets[j],float(row['delta_z']),s=68,marker=markers[j],color='#666666' if key.startswith('control_') else colors[j],edgecolor='white',linewidth=.7,zorder=3)
 if q[i]<.05:
  ax.text(i,max(values[i])+.035,'*',ha='center',va='bottom',fontsize=24,color='#111111')
if args.with_controls:
 for patch in box['boxes'][-2:]:patch.set_facecolor('#dddddd');patch.set_edgecolor('#777777')
 for line in box['medians'][-2:]:line.set_color('#555555')
ax.axhline(0,color='#8a8a8a',linewidth=1,linestyle='--',zorder=0)
ax.set_xticks(range(len(order)),[f'{label}\n(n = {len(values[i])})' for i,(label,_) in enumerate(order)])
ax.set_ylabel('Overall context selectivity (Δz)',labelpad=12)
ax.set_ylim(min(-.12,min(map(min,values))-.06),max(map(max,values))+.13)
ax.set_xlim(-.65,len(order)-.35);ax.spines[['top','right']].set_visible(False)
...
footnote='Boxes: median and interquartile range; whiskers: 1.5 × IQR. Dots: individual participants.\n* BH-FDR q < .05 across six overall ROI tests; 5,000 within-run permutations, equal-weight group means.\nTU excludes subject 5 (<10 usable voxels); OFC = olfOFC. Inference pertains to the measured participants.'
if args.with_controls:footnote=footnote.replace('across six overall ROI tests','across six original ROI tests; two control tests form a separate family')+'\nGray: controls (GM only); other ROIs: GM + Odor > Rest p < .001. Marker shapes identify subjects.'
fig.text(.12,.015,footnote,fontsize=9 if args.with_controls else 10,linespacing=1.5)
